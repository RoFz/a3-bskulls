...
_handle = 0 spawn {
    ...
    if ((isNil "lobbycomplete") || (!(isNil "lobbycomplete") && (lobbycomplete == 0))) then
    {
        ...
        private _ugvVehicle = missionNamespace getVariable ["UGV", objNull];
        private _myv1 = vehicle _ugvVehicle;
        if (isNull _ugvVehicle || {isNil "_myv1"}) then
        {
            _ugvVehicle = createVehicle ["B_PTbskull_Veh_Drone_blackops_02",BS_pos, [], 30, "NONE"];
            missionNamespace setVariable ["UGV", _ugvVehicle];
            createVehicleCrew _ugvVehicle;
            _ugvVehicle setAutonomous false;
            _ugvVehicle setPilotLight false;
        } else {
            systemChat "UGV already spawned!";
        };
        private _radarVehicle = missionNamespace getVariable ["RADAR", objNull];
        private _myv2 = vehicle _radarVehicle;
        if (isNull _radarVehicle || {isNil "_myv2"}) then
        {
            _radarVehicle = createVehicle ["B_PTbskull_Veh_Radar_blackops_01",BS_pos, [], 60, "NONE"];
            missionNamespace setVariable ["RADAR", _radarVehicle];
            createVehicleCrew _radarVehicle;
        } else {
            systemChat "RADAR already spawned!";
        };
        private _samVehicle = missionNamespace getVariable ["SAM", objNull];
        private _myv3 = vehicle _samVehicle;
        if (isNull _samVehicle || {isNil "_myv3"}) then
        {
            _samVehicle = createVehicle ["B_PTbskull_Veh_SAM_blackops_01",BS_pos, [], 80, "NONE"];
            missionNamespace setVariable ["SAM", _samVehicle];
            createVehicleCrew _samVehicle;
        } else {
